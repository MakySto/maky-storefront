 ...
 *
 * The payment modules (`lib/payment/*`, payment components) call Saleor exclusively
 * through this interface instead of importing `@/checkout/lib/actions` directly. The
 * Next.js implementation lives in `@/checkout/checkout-transport-next` and is installed
 * once by `CheckoutApp`; tests install fakes via `setCheckoutTransport`.
 */
export type CheckoutTransport = {
	/** Live checkout read bypassing any host-side cache (maps to `refreshCheckoutAction`). */
...
// lock in finalize-checkout-order).
