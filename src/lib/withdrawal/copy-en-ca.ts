...
/**
 * Canadian English copy for the online withdrawal function — PREPARED, NOT WIRED.
 *
 ...
 * notice submitted from `/ca`. Rendering the form there would offer a customer a button
 * that cannot produce a legal record — the one outcome the feature exists to prevent.
 * `servesOnlineFunction()` keeps the form Slovak-only, and `/ca/odstupenie-od-zmluvy`
 ...
 * preview state is a dated gap. Canada has no equivalent duty, and the package does not
 * invent one: the 14/30 days here are a MAKY benefit under the agreed Slovak framework,
 * not a Canadian federal statutory cooling-off period. Mandatory provincial and territorial
 * cancellation rights do arise independently of it — including Quebec's statutory
 * distance-contract cancellation — but they are separate remedies, not this function.
 *
 ...
 *
 * `data/form-capabilities.en-CA.json` says `showPickupInterest: false`, and the delivered
 ...
 * ## Provenance
 *
 * Every string is the delivered editorial value, verbatim, from `data/ui.en-CA.json`
 * (`schema maky-editorial-ui/1`, `locale en-CA`), GENERATED from that JSON rather than
 * typed out — a hand-written Polish draft once drifted from the approved export in 21 of
 * 38 strings.
 *
 * `privacyHref` (`/ca/ochrana-osobnych-udajov`) and the `formExtras` block are in the delivered
 ...
 * 3. **Never submit `ca` as `market: "SK"`.** It would write a legal record naming the
 *    wrong market.
 */
export const WITHDRAWAL_COPY_EN_CA: WithdrawalCopy = {
	...
	fullName: {
		label: "Full name",
		help: "Enter the name of the person cancelling the purchase.",
	...
	itemsRequiredError: "Enter the products and quantities you are cancelling.",
