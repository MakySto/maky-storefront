...
import { renderModule } from "../../scripts/generate-routing-constants.mjs";
import { PUBLIC_ASSET_PATHS, METADATA_ROUTE_PATHS } from "./routing.generated";
 ...
 *
 * If this fails, run `pnpm generate:routing` and commit the result.
 */
describe("public asset manifest", () => {
	it("matches what is actually on disk", () => {
		const onDisk = renderModule();
		const committed = readFileSync(join(process.cwd(), "src/lib/routing.generated.ts"), "utf8");
