...
function renderInline(nodes: readonly InlineNode[], market: string, keyPrefix: string): ReactNode[] {
	return nodes.map((node, index) => {
		const key = `${keyPrefix}-${index}`;
		if (node.kind === "text") return <span key={key}>{node.value}</span>;
		if (node.kind === "bold") {
			return <strong key={key}>{renderInline(node.children, market, key)}</strong>;
		}
		return (
			<Link
				key={key}
			...
			>
				{renderInline(node.children, market, key)}
			</Link>
		);
	});
}

export function InlineText({ text, market, id }: { text: string; market: string; id: string }) {
	return <>{renderInline(parseInline(text), market, id)}</>;
}
		...
		<div className="text-text-secondary space-y-4 leading-relaxed">
			{blocks.map((block, index) => {
				...
				if (block.type === "paragraph") {
					return (
						<p key={key}>
							<InlineText text={block.data.text} market={market} id={key} />
						</p>
					);
				}
					...
					>
						{block.data.items.map((item, itemIndex) => (
							<li key={`${key}-${itemIndex}`}>
								<InlineText text={item} market={market} id={`${key}-${itemIndex}`} />
							</li>
						))}
					</List>
				);
