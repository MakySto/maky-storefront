		...
		<Accordion type="multiple" defaultValue={["description"]} className="w-full">
			{descriptionHtml && descriptionHtml.length > 0 && (
				<AccordionItemWithContext value="description" className="border-border-default">
					<AccordionTrigger className="text-text-primary py-4 text-sm font-medium hover:no-underline">
						{t("description")}
					</AccordionTrigger>
					<AccordionContent>
						<div className="prose prose-sm text-text-secondary prose-headings:text-text-primary prose-p:text-text-secondary prose-a:text-text-link prose-strong:text-text-primary max-w-none">
						...
						<div className="grid gap-3">
							{displayAttributes.map((attr) => (
								<div key={attr.name} className="flex items-start justify-between gap-4 text-sm">
									...
									<span className="text-text-primary text-right font-medium">{formatValue(attr.value)}</span>
								</div>
							))}
						</div>
					</AccordionContent>
