...
export async function HeroProductCard({
	...
	return (
		<Link
			href={product.href}
			// A caption to the photo, not a second hero (owner, 2026-09-24/25): small and light — only
			// its ground is see-through, the name and the price stay opaque — and low on the right,
			// below the roof box it names, so the box and most of the car stay in view at every
			// width. Two lines for the name at most (a `block` beside the clamp used to switch it
			// off, and the card ran to four), no category line: "Strešný box" is in the name.
			className="group bg-surface-card/75 text-text-primary hover:bg-surface-card/90 focus-visible:ring-text-inverse absolute right-6 bottom-36 hidden w-[21rem] items-center gap-3 rounded-sm border border-white/50 p-2 pr-3.5 shadow-xl transition-colors focus-visible:ring-2 focus-visible:outline-hidden lg:flex xl:right-8"
		>
			<span className="relative h-16 w-24 shrink-0">
				{product.image && (
					<Image src={product.image} alt="" fill sizes="96px" className="object-contain mix-blend-multiply" />
				)}
			</span>
			<span className="min-w-0 flex-1">
				...
				<span className="mt-0.5 line-clamp-2 text-sm leading-snug font-bold">{product.name}</span>
				{/* The price, and the arrow beside it rather than in a disc of its own: the name gets
				    that width, and fits its two lines. */}
				<span className="mt-1 flex items-center justify-between gap-2">
					{product.price && (
						<span className="text-base font-extrabold tracking-[-0.01em] tabular-nums">{product.price}</span>
					)}
					<ArrowRightIcon
						className="text-text-secondary ml-auto h-4 w-4 shrink-0 transition-transform duration-200 group-hover:translate-x-0.5"
						strokeWidth={2.25}
						aria-hidden="true"
					/>
				</span>
