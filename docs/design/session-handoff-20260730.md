...
| Human submission numbers | ~~still `WDR-<uuid>`~~ — **wrong, corrected 2026-08-01.** `WDR-` exists in no commit under `src/`; the storefront never generates the number, it reads Payload's `submissionNumber` verbatim and checks only that it is a non-empty string. The vendored pack from PR #3's head `459146a` already specifies `ODS-YYYY-NNNNNN` and its fixture already returns `ODS-2026-000001`. Payload-side only; **zero storefront files to change** |
